...
export default function InstructorScreen({ navigation }) {
  ...
  const handleBarCodeScanned = ({ data }) => {
    const match = data.match(/(.*?)(\d{6,})\s+(\w+)/);
    if (match) {
      const name = match[1].trim();
      const instructorId = match[2];
      const department = match[3];
      setScannedData({ name, instructorId, department });
    } else {
  ...
  const handleSubmit = async () => {
    if (!scannedData) {
      alert('Please scan your Instructor ID QR first.');
      return;
    }
    ...
    <View style={{ flex: 1 }}>
      {!scannedData ? (
        <View style={styles.qrWrapper}>
          <Text style={styles.scanHeader}>SCAN INSTRUCTOR QR</Text>
          ...
          <Text style={styles.instruction}>Align your Instructor QR within the box</Text>
        </View>
      ) : (
        <ScrollView contentContainerStyle={styles.formContainer}>
          ...
          <View style={styles.detailsBox}>
            ...
            <Text style={styles.detailItem}>{scannedData.instructorId}</Text>
            <Text style={styles.detailItem}>{scannedData.department}</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  scanBox: { flex: 1, justifyContent: 'center', alignItems: 'center' },
  scanLine: {
    position: 'absolute',
    top: 0,
    left: 0,
